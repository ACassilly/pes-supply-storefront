...
import { ChevronRight, Star, Truck, ShieldCheck, MapPin, Check, Package } from "lucide-react"
        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:gap-12">
          ...
          <div className="lg:w-1/2">
            <div className="relative aspect-square overflow-hidden rounded-xl border border-border bg-muted">
              ...
              <Badge className="absolute left-3 top-3 bg-primary text-xs text-primary-foreground">{product.badge}</Badge>
              {discount > 0 && <Badge className="absolute right-3 top-3 bg-sale text-xs text-sale-foreground">Save {discount}%</Badge>}
            </div>
          </div>
          ...
          <div className="flex-1 lg:w-1/2">
            ...
            <div className="mt-3 flex items-center gap-2">
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, i) => <Star key={i} className={`h-4 w-4 ${i < Math.round(product.rating) ? "fill-accent text-accent" : "fill-muted text-muted"}`} />)}
              </div>
            ...
            <div className="mt-5 rounded-lg border border-border bg-muted/30 p-4">
              ...
              {product.price >= 999 && <p className="mt-1 text-xs font-medium text-primary">Free freight on this item</p>}
            </div>
            ...
            <div className="mt-4 flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span>Order by 2 PM ET for same-day shipping</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              {product.specs.map((s) => <span key={s} className="rounded-md border border-border bg-card px-3 py-1.5 text-xs font-medium text-card-foreground">{s}</span>)}
            </div>
